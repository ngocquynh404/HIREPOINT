...
// READ ALL - Lấy tất cả thông báo của một người dùng
...
router.get('/notifications', authenticateToken, async (req, res) => {
  try {
    const savedJobs = await SavedJob.find({ user_id: req.userId });
    const notifications = savedJobs.map(job => job.notifications).flat();
