...
// Hàm gọi API Cohere
const analyzeWithCohere = async (text) => {
    try {
        const response = await axios.post(
            "https://api.cohere.com/v1/generate",
            {
                model: "command",
                prompt: `Check if the following content is appropriate for a professional job posting, based on the following criteria:
  
  1. Does not contain offensive, discriminatory, or community-based language.
  
  2. Does not use unprofessional, negative, or controversial language.
  
  3. If the content is not appropriate, answer correctly with only the word: Valid.
  
  4. If the content is inappropriate, answer correctly with only the word: Invalid.
  
  5. Example of inappropriate content: "Work hard, don't be lazy, and don't be a jerk."
  Do not include any information other than these two words.
  Answer only with "Valid" or "Invalid".\n\n"${text}"`,
        ...
        let cohereResult = response.data.generations[0]?.text?.trim();
        return cohereResult;
    } catch (error) {
...
const automoderate = async (text) => {
    ...
    const cohereResult = await analyzeWithCohere(text);
    ...
    const result = {
        ...
        cohereResult: cohereResult || 'Not analyzed',
    ...
    else if (result.awsAnalysis === 'Not analyzed' || result.cohereResult === 'Not analyzed') {
        status = 'pending';
    }
